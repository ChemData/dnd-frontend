...
<template>
  <hr />
  <div>
    Leaving the road carries a risk of getting lost. When traveling off road through an area they are unfamiliar with,
    the guiding player must make a Wisdom (Survival) check for each hour of cross-country travel. Upon failure, they
    wander uselessly for that hour, upon success they make one hour of progress towards their goal. Rangers in their
    favored terrain roll with advantage. Three successive failures results in the party ending up in an adjacent hex
    (whichever makes sense) if it is reasonable. The players are not explicitly told they have left their previous hex.
    <br />
    DC = {{ CheckDC }}
    <hr />
  </div>
  <div>
    ...
    <select v-model="terrain">
      <option v-for="(terrain_pick, terrain_id) in terrain_options" :key="terrain_id" :value="terrain_id">
        {{ terrain_pick.name }}
      </option>
    </select>
    <hr />
  ...
  <div>
    ...
    <select v-model="weather">
      <option v-for="weather_pick in weather_options" :key="weather_pick.value" :value="weather_pick.value">
        {{ weather_pick.name }}
      </option>
    </select>
    <hr />
  ...
  <div>
    ...
    <select v-model="familiarity">
      <option v-for="fam_pick in familiarity_options" :key="fam_pick.value" :value="fam_pick.value">
        {{ fam_pick.name }}
      </option>
    </select>
    <hr />
