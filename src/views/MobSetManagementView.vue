...
export default defineComponent({
  ...
  components: {
    MobSetEditModal,
    TextInputModal, DeleteModal, vSelect, SortUp, SortDown, SortBoth, Cross, Rename, Copy, Edit},
  data() {
    return {
      mobDataStore: null,
      mobSetOptions: [],
  ...
  computed: {
    formattedSetOptions(){
      let formatted = Object.keys(this.mobSetOptions).map(key => ({id: key, name: this.mobSetOptions[key].name, source: this.mobSetOptions[key].source}))
  ...
  <div class="view-div">
    ...
    <tbody>
      <tr v-for="(set, index) in formattedSetOptions" :key="index">
        ...
        <td>
          <button class="icon-button" @click="selectedSet=set; showSetEditModal=true" title="Edit set">
            <Edit class="icon"/>
          </button>
          <button class="icon-button" @click="selectedSet=set; showRenameModal=true" title="Rename set">
            <Rename class="icon"/>
          </button>
          <button class="icon-button" @click="selectedSet=set; showCopyModal=true" title="Copy set">
            <Copy class="icon"/>
          </button>
          <button class="icon-button" @click="selectedSet=set; showDeleteModal=true" title="Delete set">
    ...
    <DeleteModal :delete-target="selectedSet.name" v-if="showDeleteModal" @cancel="showDeleteModal=false" @confirm-delete="deleteSet(selectedSet.id)" />
    <TextInputModal placeholder="New name" question="What would you like to rename it to?" v-if="showRenameModal" @cancel="showRenameModal=false" @accepted="renameSet"/>
    <TextInputModal placeholder="New name" question="What would you like to name the new set?" v-if="showCopyModal" @cancel="showCopyModal=false" @accepted="copySet"/>
    <MobSetEditModal :mob-set-id="selectedSet.id" v-if="showSetEditModal" @cancel="showSetEditModal=false" @save="saveSet"/>
    <TextInputModal placeholder="New name" question="What would you like to name the new set?" v-if="showNewSetModal" @cancel="showNewSetModal=false" @accepted="newSet"/>
  </div>
