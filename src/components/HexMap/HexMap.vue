...
import { Hex, EmptyHexGrid, Point, axialRound, axialToNormal, NormalCoord } from "../../dataStructures";
...
export default defineComponent({
  ...
  methods: {
    ...
    drawLine(
      ...
      ctx.moveTo(x, y);
      ctx.lineTo(
        x + ((this.view["scale"] * HEX_HEIGHT) / 2) * Math.sin((direction / 3) * Math.PI),
    ...
    moveMap(event) {
      ...
      if (event.key == "a") {
        ...
        this.view["offset_x"] -= this.view["scale"];
        this.view["offset_x"] = Math.max(
          -(this.view["scale"] * (1.5 * (this.map.array[0].length - 1) + 2) - this.canvas.width),
          this.view["offset_x"],
        );
      } else if (event.key == "w") {
        ...
        this.view["offset_y"] -= this.view["scale"];
        this.view["offset_y"] = Math.max(
          -(this.view["scale"] * HEX_HEIGHT * (this.map.array.length + 0.5) - this.canvas.height),
          this.view["offset_y"],
        );
      }
    },
    ...
    upOnMap(event) {
      if (this.cursor.edits && !this.editable) {
        return;
      }
      let coordinates = this.clickCoordinates(event);
